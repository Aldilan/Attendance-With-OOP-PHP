...
<div class="utama">
	...
	<table cellspacing="1" align="center" border="1">
		...
		if ($bulan == $ambil_bulan) {
			$rombel = $_GET['rombel'];
			$sql2 = mysqli_query($konek, "SELECT SUM(hadir) hadir, SUM(sakit) sakit, SUM(ijin) izin, 
				SUM(alpa) alpa, nis, nama, rombel, tgl_absen, id_rombel FROM query_absen 
				WHERE rombel='$rombel' GROUP BY nis");
			$no = 0;
